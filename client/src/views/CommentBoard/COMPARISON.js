...
function createData( name) {  return { name };  }
...
//Creating dummy data
let comment1 = new Comment("CEN3031","Sanethia Thomas","I will try to cut Dr. Thomas some slack since this was her first time teaching this class and she was handed the same material every SWE professor taught before at UF. Cannot blame the curriculum on her, but she did make rookie errors with deadlines and tests. In the end, she wanted to hear feedback from students and continued to improve","3","20");
let comment2 = new Comment("CEN3031","Sanethia Thomas","The material is divided into in lectures on coding methodology and labs where you actually do software engineering and development. Her power points are basic and unengaging but the the bulk of your grade comes from the code assignments. She knows NOTHING about programing and comes off with an attitude. Prepare to work hard in lab.","50","2");
let comment3 = new Comment("MATH2000","Math Porfessor","I hate math","5","2");
let comment4 = new Comment("ENG1020","English Prof","Love english","540","32");
let comment5 = new Comment("HIS2222","History Prof","Love history","50","2");
let comment6 = new Comment("WTV2020","Dr Whatever","Whatever","50","2");
let comment7 = new Comment("IMP2030","Impossible Course","So impossible","5","22");
let comment8 = new Comment("JOU1220","Mr. Journalist","Journalist Reporting","5220","0");
let comment9 = new Comment("SPO1111","Messi","Love sports","52220","22");
let comment10 = new Comment("TEC2023","Tech Guy","So tech","10","2");
var commentDB = [comment1, comment2, comment3, comment4, comment5, comment6, comment7, comment8, comment9, comment10];
...
function CommentBoard() {
  ...
  function handleChange(e) {  this.setState({ value: e.target.value });  }
  ...
  if(window.name == "") {  titleDisplay = "No Course Code Provided"  }
  else {  titleDisplay = "Showing Results for " + window.name.toUpperCase();  }
  ...
  return (
    <div>
      <div><h1 class = "commentBoard">Comment Board</h1></div>
      <input
        onKeyDown={(ev) => {console.log(`Pressed keyCode ${ev.key}`);
                    if (ev.key === 'Enter') {
        ...
        <div class="popup" id="popup-1">
          ...
          <div class="content">
            <div class="inputPart">
              ...
              <textarea class="popUpInputClass" type="text" placeholder="Course Code" id="courseInput"/>
              <p class="lbl">Professor:</p>
              <textarea class="popUpInputProf" type="text" placeholder="Professor" id="professorInput"/>
              <p class="lbl">Comment:</p>
              <textarea class="popUpInput" type="text" placeholder="Write your Comment" id="commentInput"/>
            </div>
            <div class="cancelSubmit">
              <button class="cancel" onClick={() => togglePopup()}>Cancel</button>
              <button class="submit" onClick={() => addToDB()}>Submit</button>
            </div>
          </div>
        </div>
      ...
      <div class="addCommentParent"><button class="addComment" onClick={() => togglePopup()}>+</button></div>


      <TableContainer class = "tableContainer" component={Paper}>
      <Table id = "tabla" class = "commentTable" className={classes.table} aria-label="custom pagination table">
        <div class="titleParent">
          <div class="center"><TableHead><StyledTableRow><StyledTableCell class="title" fontSize="20">{titleDisplay}</StyledTableCell></StyledTableRow></TableHead></div>
          <div class="filterOptions">
            <TableHead><StyledTableRow><StyledTableCell class="filterBy" fontSize="10">{"Filter By Professor:"}</StyledTableCell></StyledTableRow></TableHead>
        ...
        <TableBody>
            {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row) => (
              <StyledTableRow key={row.name}>
                <TableCell class="tableCell" component="th" scope="row">{row.name}
